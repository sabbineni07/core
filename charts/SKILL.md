...
**Azure size-pair preference (explicit):** For the same family and capacity intent, prefer **`*ds_v*`** (or equivalent local-temp-disk variant such as `*adsv*` / `*pdsv*` where naming applies) over **`*s_v*`** (for example **`Standard_D4ds_v5`** over **`Standard_D4s_v5`**). Treat recommending the no-local-temp variant when a viable local-temp variant exists as an exception path: explain the blocker in **`recommended_configuration.notes`** and/or **`confidence_notes`** (policy/region availability, workload incompatibility, measured risk, or stronger metric-backed cost/perf reason).
