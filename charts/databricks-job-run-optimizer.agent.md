...
- **Selection order discipline:** First filter to eligible SKUs (allow-list ∩ `system.compute.node_types`), then rank fit → highest `_v*` suffix **among those strings** → `ads` before `ds` same gen/tier → local-temp before diskless `s_v` when applicable. Pick by **copy-paste** from eligible set only **never swap `_v5`/`_v6` or invent `ads`**.
